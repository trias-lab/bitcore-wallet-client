...
var Bitcore_ = {
  ...
  eth: require('bitcore-lib-eth'),
  try: require('bitcore-lib-tri'),
};
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(['btc', 'bch', 'eth', 'try'], coin));
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'btc';
  if (!_.includes(['btc', 'bch', 'eth', 'try'], coin)) return cb(new Error('Invalid coin'));
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes(['btc', 'bch', 'eth', 'try'], opts.coin)) return cb(new Error('Invalid coin'));
...
API.prototype.signTxProposal = function(txp, password, cb) {
  ...
  if (txp.coin === 'eth' || txp.coin === 'try') {
    var address = txp.inputs[0].address;
    var url = '/v1/eth/address_nonce/' + address;
    if (txp.coin == 'try') {
      url = '/v1/try/address_nonce/' + address;
    }
    self._doGetRequest(url, function(err, nonceObj) {
      if (err) return cb(err);
...
API.prototype.signEthTranscation = function(password, path, rawTx)  {
  var derived = this.credentials.getDerivedXPrivKey(password);
  var xpriv = new Bitcore.HDPrivateKey(derived);
  var privateKey = xpriv.deriveChild(path).privateKey;

  var bitcore = Bitcore_['eth'];
  var serializedTx =  bitcore.Transaction.signEthTranscation(privateKey, rawTx.to, rawTx.data, rawTx.value, rawTx.gasPrice, rawTx.nonce, rawTx.gasLimit);
  return serializedTx;
};

/*
   查询交易的pending状态。用法如下：
        var bwcClient //这是bwc的客户端
        var opts = {
          'coin':'try' //币种名，try,eth,btc
          'network':'testnet', //网络 testnet 或者 livenet
          'txid':'0xabcd...' //发送交易后获得的交易id
        }
        bwcClient.getTransactionPendingStatus(opts, function(err, status) {
             //err 如果出错，err不为空。如果不出错，err为null。
             //如果err === null，则status代表交易的状态，目前有两种
             //'confirmed'  代表交易已经上链了，可以在区块链浏览器中查到
             //'pending' 代表交易还没有上链
        })
 */
API.prototype.getTransactionPendingStatus = function(opts, cb) {
  var self = this;
  opts = opts || {};

  if (!opts.coin) {
    return cb(new Error('can not find coin param'));
  }
  opts.coin = opts.coin.toLowerCase()
  if(opts.coin !== 'btc' &&  opts.coin !== 'eth' && opts.coin !== 'try') {
    return cb(new Error('coin param not supported: ' + opts.coin));
  }
  if (!opts.network) {
    return cb(new Error('can not find network param'));
  }
  opts.network = opts.network.toLowerCase()
  if(opts.network !== 'livenet' &&  opts.network !== 'testnet') {
    return cb(new Error( 'network param not supported: ' + opts.network));
  }
  if (!opts.txid) {
    return cb(new Error('can not find txid param'));
  }
  opts.txid = opts.txid.toLowerCase()

  var qs = [];
  qs.push('coin=' + opts.coin);
  qs.push('network=' + opts.network);
  qs.push('txid=' + opts.txid);

  self._doGetRequest('/v1/get_transaction_pending_status?' + qs.join('&'), function(err, result) {
    if (err) return cb(err);
    if (result.status.length > 0) {
      return cb(null, result.status)
    }
    return cb(new Error('bws error: ' + result,message));
  });
};
