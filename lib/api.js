...
API.prototype.signEthTranscation = function(password, path, to, data, value, gasPrice, nonce, gasLimit)  {
  var derived = this.credentials.getDerivedXPrivKey(password);
  var xpriv = new Bitcore.HDPrivateKey(derived);
  var privateKey = xpriv.deriveChild(path).privateKey;

  var bitcore = Bitcore_['eth'];
  var serializedTx =  bitcore.Transaction.signEthTranscation(privateKey, to, data, value, gasPrice, nonce, gasLimit);
};
