...
Utils.formatAmount = function(satoshis, unit, opts) {
  ...
  if (opts.coin && (opts.coin === 'eth' || opts.coin === 'try')) {
    u = {
      toSatoshis: 10**18,
      full: {
        maxDecimals: 18,
...
Utils.buildTx = function(txp) {
  ...
  t.fee(txp.fee);
  if ((txp.coin === 'eth' || txp.coin === 'try') &&(!txp.changeAddress || !txp.changeAddress.address)) {
    txp.changeAddress = {}
  ...
  if (txp.coin !== 'eth' && txp.coin !== 'try' && t.outputs.length > 1) {
