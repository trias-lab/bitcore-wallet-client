...
Utils.formatAmount = function(satoshis, unit, opts) {
  ...
  if (opts.coin && (opts.coin === 'eth' || opts.coin === 'tri')) {
    var u1 = {
      toSatoshis: 10**18,
      full: {
        maxDecimals: 18,
    ...
    var amount = clipDecimals((satoshis / u1.toSatoshis), u1[precision].maxDecimals).toFixed(u1[precision].maxDecimals);
  } else {
...
Utils.buildTx = function(txp) {
  ...
  t.fee(txp.fee);
  if ((txp.coin === 'eth' || txp.coin === 'tri') &&(!txp.changeAddress || !txp.changeAddress.address)) {
    txp.changeAddress = {}
    txp.changeAddress.address = txp.outputs[0].toAddress;
  }
  t.change(txp.changeAddress.address);

  // Shuffle outputs for improved privacy
  if (txp.coin !== 'eth' && txp.coin !== 'tri' && t.outputs.length > 1) {
